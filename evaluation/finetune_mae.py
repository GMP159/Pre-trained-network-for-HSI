...
def main():
    ...
    parser.add_argument(
        "--use_pretrained",
        action="store_true",
        help="Load MAE checkpoint before fine-tuning",
    )
    ...
    print("Fine-tuning FULL model (single GPU)")
    ...
    best_acc = 0.0
    best_path = (
        "outputs/finetune_mae_best.pth"
        if args.use_pretrained
        else "outputs/finetune_mae_random_best.pth"
    )
    ...
    print(
        f"Complete! Best: {best_acc:.2f}% "
        f"({'pretrained' if args.use_pretrained else 'random'})"
    )
