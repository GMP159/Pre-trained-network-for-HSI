#!/usr/bin/env python
"""
Linear probing with Contrastive pre-trained encoder
"""
...
# Config
CONTRASTIVE_CHECKPOINT = "outputs/pretrain_enhanced/contrastive/checkpoint_epoch0150.pth"
...
print("="*80)
print("LINEAR PROBING - Contrastive Pre-trained Encoder")
print("="*80)
...
# Load Contrastive pre-trained model
print("\nLoading Contrastive pre-trained model...")
model = MaskedSST(
    img_size=64,
    patch_h=4,
    patch_w=4,
...
# Training loop
best_acc = 0.0
results_file = Path("outputs/linear_probe_contrastive_results.txt")
...
for epoch in range(NUM_EPOCHS):
    ...
    if test_acc > best_acc:
        best_acc = test_acc
        torch.save(classifier.state_dict(), 'outputs/apple_classifier_contrastive_best.pth')
...
with open(results_file, 'w') as f:
    f.write(f"Contrastive Linear Probing Results\n")
    f.write(f"{'='*80}\n")
    f.write(f"Checkpoint: {CONTRASTIVE_CHECKPOINT}\n")
